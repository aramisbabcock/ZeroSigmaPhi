...
import org.cucumbertaf.utils.ExcelReader;
import org.cucumbertaf.utils.PropertyUtil;
import org.cucumbertaf.utils.ExtentReportingService;
...
public class Hooks {
	...
	public void before(Scenario scenario) throws Exception {
		...
		String featureNameTemp = this.testContext.getFeatureName()
				.split("/")[this.testContext.getFeatureName().split("/").length - 1].replaceAll(".feature", "");
		...
		ExcelReader reader = new ExcelReader(this.testContext.getFeatureName(), this.testContext.getScenarioName(),
				iteration_select);
		...
		if (extent.get() == null) {
			...
			eTestThreadLocal
					.set(extent.get().createTest(featureNameTemp + "_" + dataExl.get("workflowdescription"), info));
		...
		if (current_iteration % total_scenarios_count == 0) {
			Globals.counterTracker.get().put("current_iteration",
					Globals.counterTracker.get().get("current_iteration") + 1);
		}
	...
	public void afterStep(Scenario scenario) throws IOException {
		...
		if (scenario.getStatus() == io.cucumber.java.Status.PASSED) {
			...
			String dest_path = "./test-output/" + ExtentReportingService.getFolderNameTimeStamp() + "/screenshots/"
					+ curr_step_name + "_" + time_stamp + ".png";
			FileHandler.copy(screenshot, new File(dest_path));
			String pathOfHtmlScreenshot = "screenshots/" + curr_step_name + "_" + time_stamp + ".png";
			this.testContext.getExtentTest().log(Status.INFO,
					MediaEntityBuilder.createScreenCaptureFromPath(pathOfHtmlScreenshot).build());
			...
			this.testContext.getExtentTest().log(Status.SKIP,
					scenario.getName() + " is skipped and the reason for the skip is mentioned above");
			File screenshot = ((TakesScreenshot) this.testContext.getDriver()).getScreenshotAs(OutputType.FILE);
			String dest_path = "./test-output/" + ExtentReportingService.getFolderNameTimeStamp() + "/screenshots/"
					+ curr_step_name + "_" + time_stamp + ".png";
			FileHandler.copy(screenshot, new File(dest_path));
	...
	public void after(Scenario scenario) throws Exception {
		...
		/*
		 * if (scenario.getStatus() == io.cucumber.java.Status.PASSED) { File screenshot
		 * = ((TakesScreenshot)
		 * this.testContext.getDriver()).getScreenshotAs(OutputType.FILE); String
		 * dest_path = "./test-output/" +
		 * ExtentReportingService.getFolderNameTimeStamp() + "/screenshots/" +
		 * curr_step_name + "_" + time_stamp + ".png"; FileHandler.copy(screenshot, new
		 * File(dest_path)); String pathOfHtmlScreenshot = "screenshots/" +
		 * curr_step_name + "_" + time_stamp + ".png";
		 * this.testContext.getExtentTest().log(Status.PASS,
		 * MediaEntityBuilder.createScreenCaptureFromPath(pathOfHtmlScreenshot).build())
		 * ; } else if (scenario.getStatus() == io.cucumber.java.Status.FAILED) { File
		 ...
		 * File(dest_path)); String pathOfHtmlScreenshot = "screenshots/" +
		 * curr_step_name + "_" + time_stamp + ".png";
		 * this.testContext.getExtentTest().log(Status.FAIL,
		 * MediaEntityBuilder.createScreenCaptureFromPath(pathOfHtmlScreenshot).build())
		 * ; this.testContext.getExtentTest().fail(Globals.error); for (int i =
		 * stepCount; i < testSteps.size(); i++) { curr_step_name =
		 * testSteps.get(i).getStep().getText();
		 * this.testContext.getExtentTest().log(Status.SKIP, curr_step_name +
		 * " step is skipped.....!"); } } else if (scenario.getStatus() ==
		 * io.cucumber.java.Status.SKIPPED) {
		 * this.testContext.getExtentTest().log(Status.SKIP, curr_step_name +
		 * " step is skipped.....!"); } else if (scenario.getStatus() ==
		 * io.cucumber.java.Status.UNDEFINED) {
		 * this.testContext.getExtentTest().log(Status.WARNING, curr_step_name +
		 * " step is undefined.....!"); } else if (scenario.getStatus() ==
		 * io.cucumber.java.Status.PENDING) {
		 * this.testContext.getExtentTest().log(Status.WARNING, curr_step_name +
		 * " step is pending.....!"); } else if (scenario.getStatus() ==
		 * io.cucumber.java.Status.AMBIGUOUS) {
		 * this.testContext.getExtentTest().log(Status.WARNING, curr_step_name +
		 * " step is ambiguous.....!"); } else {
		 * this.testContext.getExtentTest().log(Status.WARNING, curr_step_name +
		 * " step is not defined.....!"); }
		 */
		...
		if (current_iteration == 1 || total_scenarios_count == 1
				|| ((current_iteration - 1) % total_scenarios_count == 0)) {
			extent.get().flush();
			extent.set(null);
			extent.remove();
		...
		int iteration_select = Float.valueOf(this.testContext.getExl_write_data_map().get("iteration_write").toString())
				.intValue();
		ExcelReader writter = new ExcelReader(this.testContext.getFeatureName(), this.testContext.getScenarioName(),
				iteration_select);
